...
Bank accounts are distinct from chart of accounts entries. The chart of accounts has a general ledger account (e.g., "1010 - Cash - Operating") that tracks the balance according to your books. The bank account represents the same account from the bank's perspective. Reconciliation (see [Bank Reconciliation](bank-reconciliation.md)) is the process of verifying these two views agree.

## Navigating to bank accounts

The Bank Accounts page is located at **Accounting > Bank Accounts**. You can also reach it from the Accounting dashboard by clicking the "Bank Accounts" card.

> **Screenshot placeholder:** *Bank Accounts page showing a table of bank accounts with columns for name, institution, linked account, balance, and transaction count.*
...
> **Screenshot placeholder:** *Bank Account detail page showing the summary cards, linked account badge, and transaction table with matched/unmatched status badges.*
...
- **Real-time balance updates** -- Account balances are synced periodically with your bank

The data model already supports Plaid fields (`plaidItemId`, `plaidAccessToken`, `plaidAccountId`, `lastSyncedAt`) on bank accounts, and `plaidTransactionId` on bank transactions. When enabled, the Plaid integration will populate these fields automatically.

Until Plaid is enabled, bank transactions can be entered manually or imported from other sources.
