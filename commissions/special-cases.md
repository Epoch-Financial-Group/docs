...
Special case groups provide an additional layer of conditional logic for commission rules. While regular commission rule filters match on a single attribute at a time (one lender, one loan type, etc.), special case groups let you define multi-condition checks that must be satisfied before a rule applies.

## What Are Special Case Groups?

A **CommissionSpecialCaseGroup** is a named collection of criteria (conditions) attached to a commission template. Each group contains one or more **CommissionSpecialCase** items -- individual conditions that are evaluated against a loan's attributes.
...
<!-- Screenshot placeholder: Special case group section in template editor -->
> **[Screenshot]** The Special Cases section showing an empty group ready for criteria to be added.
...
- **AND** (operator = true): Both the accumulated result and this criterion must be true.
- **OR** (operator = false): Either the accumulated result or this criterion must be true.
...
Special case groups are part of a commission template and are linked to commission rules:

```
CommissionTemplate
  |
  +--> CommissionSpecialCaseGroup
  |      |
  |      +--> CommissionSpecialCase (criterion 1)
  |      +--> CommissionSpecialCase (criterion 2)
  |      +--> ...
  |
  +--> CommissionRule (linked to the group)
  +--> CommissionRule (not linked -- uses standard filters only)
```

A single special case group can be linked to multiple rules within the same template. When the group's criteria are met, all linked rules become eligible for matching (subject to their own filters and the rule precedence system described in [Commission Rules](commission-rules.md)).
