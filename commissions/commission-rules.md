...
Commission rules override the base commission defined on a template when a loan matches specific criteria. Rules let you define different commission amounts for different loan scenarios -- for example, a higher rate for purchase loans, a lower rate for VA loans, or a flat fee for loans from a specific lender.
...
<!-- Screenshot placeholder: Rule form within template editor -->
> **[Screenshot]** The rule editor form showing commission settings at the top and filter sections below.
...
1. **Filter by role flags**: Only rules matching the current role context are considered (LO rules for LO calculation, processor rules for processor calculation, etc.).
2. **Filter by employee**: If the rule is bound to a specific loan officer (via `loanOfficerId`), it must match the loan's LO. Rules with no `loanOfficerId` are template defaults.
3. **Sort by specificity**: Rules are sorted with more specific rules first:
   - Rules with a `loanOfficerId` set rank higher than rules without one
   - Rules with a `recipientId` set rank higher than rules without one
4. **Evaluate filters**: Each rule's filters are checked against the loan. All filters must match (AND logic).
5. **Check special cases**: If the rule belongs to a special case group, the group's conditions must also be satisfied.
6. **First match wins**: The first rule that passes all checks is used.
...
When a template is assigned to an employee, both the base commission and all rules are copied into employee-bound rules. The engine prioritizes employee-bound rules (which have `loanOfficerId` set) over template-level rules (which do not).

This means if an admin creates an employee-specific override rule directly on the employee, it will take precedence over the template-generated rules.
